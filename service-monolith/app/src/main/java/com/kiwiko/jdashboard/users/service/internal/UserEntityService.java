...
public class UserEntityService implements UserService {
    ...
    public GetUsersByQueryResponse getByQuery(GetUsersQuery query) {
        Set<com.kiwiko.jdashboard.users.client.api.dto.User> users = transactionProvider.readOnly(() -> userEntityDAO.getByQuery(query).stream()
                .map(mapper::toDto)
    ...
    @Override
    public com.kiwiko.jdashboard.users.client.api.dto.User toUser(User user) {
